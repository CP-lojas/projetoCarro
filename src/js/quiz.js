...
document.addEventListener('DOMContentLoaded',()=>{
    const mensagem = document.getElementById('message');
    const containerResultado = document.getElementById('container-resultado');
    const mostreGabarito = document.getElementById("mostre");
    const listaResultado = document.getElementById('lista-resultado');
    const reiniciarBotao = document.getElementById('inicio-btn')
    ...
    function mostrarPergunta(){
        if(perguntas <questoes.length){
            pergunta.textContent =gabarito[perguntas];
            resposta.value='';
            mensagem.textContent ='';
        }else{
            mostrarResultado();
    ...
    function mostrarResultado(){
        ...
        gabarito.forEach((questoes,index)=>{
            const listaItem =document.createElement('li');
            listaItem.innerHTML = `<strong>${gabarito}</strong><br>
            Sua Resposta: <span>${respostas[index]}</span>`
            listaResultado.appendChild(listaItem);
        })
    }
    ...
    proximaPergunta.addEventListener('click', Mostrargabarito);
    reiniciarBotao.addEventListener('click',reiniciarQuiz);
